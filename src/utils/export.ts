...
import { formatFileSize, formatDateTime, formatNumber } from './format';
...
export function exportDuplicateGroups(groups: DuplicateGroup[], filename: string = '重复文件清单'): void {
  const flattened = groups.flatMap(group =>
    group.files.map((file, index) => ({
      groupId: group.id,
      hash: group.hash,
      fileCount: formatNumber(group.fileCount),
      ...
      fileSizeBytes: formatNumber(file.size),
...
export function exportDepartmentReports(reports: DepartmentReport[], filename: string = '部门整改清单'): void {
  const data = reports.map(r => ({
    rank: r.rank,
    name: r.name,
    duplicateCount: formatNumber(r.duplicateCount),
    duplicateSize: formatFileSize(r.duplicateSize),
    duplicateSizeBytes: formatNumber(r.duplicateSize),
    saveableSize: formatFileSize(r.saveableSize),
    saveableSizeBytes: formatNumber(r.saveableSize),
    fileCount: formatNumber(r.fileCount),
    totalFileCount: formatNumber(r.totalFileCount),
    duplicateRate: (r.duplicateRate * 100).toFixed(1) + '%',
    duplicateRateValue: (r.duplicateRate * 100).toFixed(1),
    pendingRecycleCount: formatNumber(r.pendingRecycleCount),
    pendingRecycleSize: formatFileSize(r.pendingRecycleSize),
    pendingRecycleSizeBytes: formatNumber(r.pendingRecycleSize),
    rectificationCount: formatNumber(r.rectificationCount),
    rectificationPending: formatNumber(r.rectificationPending),
    rectificationConfirmed: formatNumber(r.rectificationConfirmed),
    rectificationRejected: formatNumber(r.rectificationRejected),
    head: r.head,
...
export function exportRecycleItems(items: { fileItem: FileItem; movedAt: string; movedBy: string; reason: string }[], filename: string = '待回收文件清单'): void {
  const data = items.map(item => ({
    ...
    fileSizeBytes: formatNumber(item.fileItem.size),
